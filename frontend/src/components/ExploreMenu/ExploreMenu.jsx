...
const ExploreMenu = ({ category, setCategory }) => {
    ...
    useEffect(() => {
        ...
        const fetchCategories = async () => {
            ...
            try {
                ...
                if (response.data.success) {
                    // Process each category to find an appropriate image
                    const formattedCategories = response.data.categories.map((cat) => {
                        // First try to find an item from this category to use its image
                        const itemsInCategory = food_list.filter(item => item.category === cat);
                        let menuImage;
                        
                        if (itemsInCategory.length > 0) {
                            // Use the image from the first item in this category
                            const firstItem = itemsInCategory[0];
                            if (firstItem.image) {
                                // If the image is a data URI, use it directly
                                if (firstItem.image.startsWith('data:')) {
                                    menuImage = firstItem.image;
                                } else {
                                    // Otherwise, use the URL from the backend
                                    menuImage = `${url}/images/${firstItem.image}`;
                                }
                            }
                        }
                        
                        // If no food item image is available, fall back to predefined icons
                        if (!menuImage) {
                            ...
                            switch (cat.toLowerCase()) {
                                ...
                                case 'dessert':
                                    ...
                                    break;
                                default:
                                    // Generate a simple default icon for new categories
        ...
        fetchCategories();
    }, [url, food_list]); // Re-fetch when food_list changes too
